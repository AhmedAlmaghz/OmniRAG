...
> **حالة التنفيذ (تحديث ختامي):** المراحل 1، 2، 4، 5 **منفذة بالكامل**، والمرحلة 3 منفذة في معظمها: جوال المحادثة + a11y التبويبات + React Query (Plans وKnowledgeBase) + أقسام SettingsView lazy + إلغاء KB_TABS المكرر + هجرة دفعة أولى من نصوص KB إلى قواميس `kb.*` (19 نصًا). **المتبقي فعليًا:** ~350 نصًا ثنائيًا مضمّنًا متبقيًا في McpGateway/Analytics/LandingPage/SettingsView وبقية KnowledgeBase (ميكانيكي بحت، النمط والبنية جاهزان في namespace `kb.*`).
