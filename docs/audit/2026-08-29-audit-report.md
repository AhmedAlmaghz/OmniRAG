...
6. **CSRF رفيع على غير مسارات المصادقة** — ترويسة `x-requested-with` قابلة للتزييف من أي سكربت؛ كل مسارات تعديل الحالة غير المصادقية تعتمد على `SameSite=Lax` وحده.
   **[معالج v0.12.12]** مسارات المصادقة الأربعة (login/register/logout/sso-initiate) — التي لا يمكنها الجلوس خلف `withAuthAndRateLimit` لأنه يصادق أولاً — أصبحت تطبّق بوابة Origin القوية ذاتها (`originGuardDenied` في `src/lib/api/originGuard.ts`) فوق حارس الترويسة القديم كطبقة ثانية، مع اختبار `authOriginGate.test.ts` (8 فحوص).
7. **قراءات كاملة غير مقسمة** — `GET /api/v1/documents` يعيد كل محتوى المستندات (حتى 4M حرف للمستند)؛ `?documentId=` يحمّل كل chunks ثم يرشّح في JS؛ `/api/v1/sources` يعيد كل سجلات المزامنة. مع جلسة/مفتاح مسروق = exfiltration بطلب واحد.
   **[معالج v0.12.11]** القائمة أصبحت ملخصات (بلا محتوى ولا إصدارات — طول + معاينة 400 حرف) مع ترقيم صفحات (`limit/offset`) وجلب تفصيلي لكل مستند (`?id=` يُرجع المحتوى الكامل والإصدارات)، وسجلات المزامنة مقيّدة (`?syncLogLimit=` افتراضي 100)، ومسار chunks كان مقسماً SQL-ياً من قبل. المثبت حياً على Postgres 17: `content_len=0` في القائمة بينما التفصيل يعيد 693 حرفاً + الإصدارات.
...
9. **درع حقن التعليمات regex-based** — `hook-harness.ts:24-46` يُتجاوز بإعادة الصياغة؛ جيد كطبقة، خطر إن عُدّcontrolًا.
   **[موثّق v0.12.12]** الحد معروف معماريًا: الدرع طبقة حتمية رخيصة تلتقط الأنماط الساذجة وليس ضابط أمان مطلق — لا يجوز الاعتماد عليه وحده. الاعتماد الفعلي في التصميم على الطبقات المتعاقبة (H1 TenantGate، موافقة بشرية على أدوات T2/T3، تنقيح PII — راجع ملخّص طبقات الدفاع في `docs/06-security/overview.md`). أي ترقية دلالية (مصنّف LLM) مرفوضة عمداً في هذه المرحلة لتكلفتها وزمن استجابتها — راجع خارطة الطريق.
...
11. Swagger UI من unpkg CDN بلا CSP على `/api/docs`.
    **[معالج]** `buildDocsCsp` في `src/lib/security/securityHeaders.ts` يمنح CSP مخصصاً لصفحة Swagger وحدها (`script-src 'self' https://unpkg.com 'nonce-…'`) — تحقق v0.12.12.
